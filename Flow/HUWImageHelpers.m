...
@interface HUWImageHelpers ()

@end

@implementation HUWImageHelpers
...
+(CALayer*)reflectionLayer:(UIImageView*)imageView withHeight:(NSUInteger)height andDistance:(NSUInteger)distance {
    UIImage *reflection = [HUWImageHelpers reflectedImage:imageView withHeight:height];
    CALayer *sublayer = [CALayer layer];
    sublayer.contents = (id)reflection.CGImage;
    sublayer.opacity = 0.3;
    sublayer.frame = CGRectMake(0, imageView.frame.size.height + distance, imageView.frame.size.width, reflection.size.height);
    return sublayer;
}
